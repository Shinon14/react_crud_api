...
const CompanyForm = () => { 
    ...
    const getCompany = async (companyId) => {
        try {
            ...
            const data = await res.json();
            const { name, foundation, webiste } = data.company;
            setCompany({ name, foundation, webiste });
        } catch (error) {
            console.log(error)
        };
    };
        ...
        <div className="col-md-3 mx-auto">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="mb-3">
                    ...
                    <input type="number" name="foundation" value={company.foundation} onChange={handleInputChange}  className="form-control" min="1900" max="2080" required />
                </div>
                <div className="mb-3">
                    <label className="form-label">Website</label>
                    <input type="url" name="webiste" value={company.website} onChange={handleInputChange}  className="form-control" maxLength="100" required />
                </div>
